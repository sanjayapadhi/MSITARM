...
function PerformActionOnSequencedTaggedVMs($Sequences, [string]$Action, $TagName, [string[]]$VMRGList, $ExcludeList)
{
    $EmailVMList=@()
    ...
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
			...
			{
                $AzureVMList += Find-AzureRmResource -TagName $TagName.ToLower() -TagValue $seq | Where-Object {($_.ResourceType -eq “Microsoft.Compute/virtualMachines”) -and ($_.ResourceGroupName -eq $rg)} | Select Name, ResourceGroupName
            }
            ...
            Write-Output "Completed the sequenced $($Action) against VM's where the tag SequenceStart is $($seq)." 
...
try
{
	...
	if ($Action -eq 'start')
	{
		PerformActionOnSequencedTaggedVMs -Sequences $startSequences -Action $Action -TagName $startTagValue -VMRGList $VMRGList -ExcludeList $VMfilterList
	}
	
	if ($Action -eq 'stop')
	{
		PerformActionOnSequencedTaggedVMs -Sequences $stopSequences -Action $Action -TagName $stopTagValue -VMRGList $VMRGList -ExcludeList $VMfilterList
	}
